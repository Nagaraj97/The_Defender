...
float gTranslationX = 0.0f;  
float gTranslationY = 0.0f;  
...
void DrawGear()
{
    ...
    for (int i = 0; i <= kTeethCount; ++i)
    {
        ...
        glVertex2f((0.7f * kRadius) * cos(angle + kAngleStep / 2.0f), (0.7f * kRadius) * sin(angle + kAngleStep / 2.0f));
    }
    glEnd();
}
...
void Display()
{
    ...
    glPushMatrix();
    glTranslatef(gTranslationX, gTranslationY, -0.2f); 
    glRotatef(gRotationAngle, 0.0f, 0.0f, 1.0f);       
...
void updateBall(int value)
{
    ...
    if (ballY < -10.0 + ballRadius || ((int)ballY == (int)gTranslationY & (int)ballX == (int)gTranslationX))
    {
